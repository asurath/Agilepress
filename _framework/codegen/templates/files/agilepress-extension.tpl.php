...
define( 'APEXT_DISPLAY_PATH', APEXT_PATH . '/display' );
define( 'APEXT_CONSTANTS_PATH', APEXT_PATH . '/constants' );
define( 'APEXT_USER_TYPES_PATH', APEXT_PATH . '/user-types' );
...
define( 'APEXT_TAXONOMY_PATH', APEXT_PATH . '/taxonomy/' );
define( 'APEXT_PAGES_PATH', APEXT_PATH . '/pages/' );
define( 'APEXT_CONTENT_PATH', APEXT_PAGES_PATH . '/content/' );
...
foreach($this->objConfigData->posttypes->posttype as $objPostType){
	...
	$strComma = ($i < $len - 1) ? "," : "";
	echo "\t '". AP_PREFIX . str_replace(" ", "" , $objPostType[0]['arguments']['labels']['singular_name']) . "Type'" . $strComma . " \n";
}
		?>
		)
	)
...
// Custom User Types
define ( 'AP_CUSTOM_USER_TYPES', 
	serialize(
		array(
		'<?= AP_PREFIX ?>UserType',
		<?php 
$i = 0;
$len = count($this->objConfigData->usertypes->usertype);
foreach($this->objConfigData->usertypes->usertype as $objUserType){
	$objUserType = unserialize($objUserType);
	$strComma = ($i < $len - 1) ? "," : "";
	echo "\t '". AP_PREFIX  . str_replace(" ", "" , $objUserType[0]['singular_name']) . "Type'" . $strComma . " \n";
}
		?>
		)
	)
...
// Custom Taxonomies
define ( 'AP_TAXONOMIES', 
	serialize(
		array(
		<?php 
$i = 0;
$len = count($this->objConfigData->taxonomies->taxonomy);
foreach($this->objConfigData->taxonomies->taxonomy as $objTaxonomy){
	$objTaxonomy = unserialize($objTaxonomy);
	$strArgumentArray = unserialize($objTaxonomy['arguments']);
	$strComma = ($i < $len - 1) ? "," : "";
	echo "\t '". AP_PREFIX . str_replace(" ", "" , $strArgumentArray['labels']['singular_name']) . "Type'" . $strComma . " \n";
}
		?>
		)
	)
...
// Run your app!
if (AP_IsActive() && AP_IsExtensionActive() && AP_PluginActivationCheck()) {
...
//////////////
	...
	require_once( APEXT_POST_PATH . '/generated/AP_PostItemGen.class.php');
	...
	foreach($this->objConfigData->posttypes->posttype as $objPostType){
		$objPostType = unserialize($objPostType);
		$objPostType[0]['arguments']['labels']['singular_name'] = str_replace(" ", "" , $objPostType[0]['arguments']['labels']['singular_name']);
		echo "\t// Includes for CustomPostType ". $objPostType[0]['arguments']['labels']['singular_name']. " \n";
		echo "\t\t require_once(APEXT_POST_TYPES_PATH . '/generated/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "TypeGen.class.php'); \n";
		echo "\t\t require_once(APEXT_POST_PATH . '/generated/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "ItemGen.class.php'); \n";
		echo "\t\t require_once(APEXT_DISPLAY_PATH . '/generated/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "DisplayGen.class.php'); \n";
		echo "\t\t require_once(APEXT_POST_TYPES_PATH . '/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "Type.class.php'); \n";
		echo "\t\t require_once(APEXT_POST_PATH . '/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "Item.class.php'); \n";
		echo "\t\t require_once(APEXT_DISPLAY_PATH . '/AP_" . $objPostType[0]['arguments']['labels']['singular_name'] . "Display.class.php'); \n";
	...
	foreach($this->objConfigData->taxonomies->taxonomy as $objTaxonomy){
		$objTaxonomy = unserialize($objTaxonomy);
		$strArgumentArray = unserialize($objTaxonomy['arguments']);
		$strPostTypeArray = unserialize($objTaxonomy['post_types']);
		$strArgumentArray['labels']['singular_name'] = str_replace(" ", "" , $strArgumentArray['labels']['singular_name']);
		echo "\t// Includes for CustomTaxonomy ". $strArgumentArray['labels']['singular_name']. " \n";
		echo "\t\t require_once(APEXT_TAXONOMY_PATH . '/generated/AP_" . $strArgumentArray['labels']['singular_name'] . "TypeGen.class.php'); \n";
		echo "\t\t require_once(APEXT_TAXONOMY_PATH . '/AP_" .$strArgumentArray['labels']['singular_name'] . "Type.class.php'); \n";
	}
	?>
...
// ACTIVATION ORDER
// =========================================
//register_activation_hook( __FILE__, 'AP_PluginActivationCheck' );
add_action( 'admin_init', 'AP_PluginActivationCheck' );

/**
 * Main function that checks if the plugin should be activated
 */
function AP_PluginActivationCheck(){
	if (AP_IsActive()) {
		$mixPluginsArray = get_option( 'active_plugins' ); // Array of active plugins (this is a WordPress core option)		
		$ap_plugin_basename = AP_GetPluginBasename();
		
		
		// AgilePress is active! Let's make sure it's at the front of the line now
		if ( $key = array_search( $ap_plugin_basename, $mixPluginsArray ) ) {
			array_splice( $mixPluginsArray, $key, 1 );
			array_unshift( $mixPluginsArray, $ap_plugin_basename );
			update_option( 'active_plugins', $mixPluginsArray );
			return false; 
		}
	}
	else {
		// AgilePress is not active, add a notice so the user knows to activate AgilePress
		add_action( 'admin_notices', 'AP_DeactivatedNotice');
	}
	
	AP_AddExtensionSetting();
 ...
 * @return boolean
 */
function AP_IsActive() {
	$mixPluginsArray = get_option( 'active_plugins' );
	$ap_plugin_basename = AP_GetPluginBasename();
	
	if (array_search($ap_plugin_basename, $mixPluginsArray) === false)
		return false;
	return true;
}
 ...
 * @return boolean
 */
function AP_IsExtensionActive() {
	$mixPluginsArray = get_option( 'active_plugins' );
	
	if (array_search(APEXT_BASENAME, $mixPluginsArray) === false)
		return false;
	return true;
}
 ...
 */
function AP_AddExtensionSetting() {
 ...
 */
function AP_DeactivateExtensionPlugin() {
	if ($mixPluginsArray = get_option( 'active_plugins' )) {
	
		$key = array_search( APEXT_BASENAME, $mixPluginsArray );
