...
class AP_DateTime extends DateTime {
	...
	protected $boolDateNull = true;
	protected $boolTimeNull = true;
	public static $boolTranslate = false;

	const NOW = 'now';
	const FORMAT_ISO = 'YYYY-MM-DD hhhh:mm:ss';
	const FORMAT_ISO_COMPRESSED = 'YYYYMMDDhhhhmmss';
	const FORMAT_DISPLAY_DATE = 'MMM DD YYYY';
	const FORMAT_DISPLAY_DATE_FULL = 'DDD, MMMM D, YYYY';
	const FORMAT_DISPLAY_DATE_TIME = 'MMM DD YYYY hh:mm zz';
	const FORMAT_DISPLAY_DATE_TIME_FULL = 'DDDD, MMMM D, YYYY, h:mm:ss zz';
	const FORMAT_DISPLAY_TIME = 'hh:mm:ss zz';
	const FORMAT_RFC822 = 'DDD, DD MMM YYYY hhhh:mm:ss ttt';

	public static $DefaultFormat = AP_DateTime::FORMAT_DISPLAY_DATE_TIME;
	public static $DefaultTimeOnlyFormat = AP_DateTime::FORMAT_DISPLAY_TIME;
	public static $DefaultDateOnlyFormat = AP_DateTime::FORMAT_DISPLAY_DATE;

	/**
	 * Returns a new AP_DateTime object that's set to "NOW"
	 * Set boolTimeValue to true (default) for a DateTime, and set boolTimeValue to false for just a Date
	 *
	 * @param boolean $boolTimeValue whether or not to include the time value
	 * @return AP_DateTime the current date and/or time
	 */
	public static function Now($boolTimeValue = true) {
		$dttToReturn = new AP_DateTime(AP_DateTime::NOW);
		if (!$boolTimeValue) {
			$dttToReturn->boolTimeNull = true;
	...
	protected function AP_DateTimeErrorHandler() {}
	...
	public function __construct($mixValue = null, DateTimeZone $objTimeZone = null) {
		...
		if (strtolower($mixValue) == AP_DateTime::NOW) {
			...
			$this->boolDateNull = false;
			$this->boolTimeNull = false;
			...
			$intTimestamp = null;
			$boolValid = false;
			set_error_handler(array($this, 'AP_DateTimeErrorHandler'));
			try {
				if ($objTimeZone)
					$boolValid = parent::__construct($mixValue, $objTimeZone);
				else
					$boolValid = parent::__construct($mixValue);
	 ...
	 * it will return as AP_DateTime::FORMAT_DISPLAY_DATE "MMM DD YYYY", e.g. Mar 20 1977.
	 *
	...
	public function IsDateNull() {
		return $this->boolDateNull;
	}
	public function IsNull() {
		return ($this->boolDateNull && $this->boolTimeNull);
	}
	public function IsTimeNull() {
		return $this->boolTimeNull;
	}
	...
	protected function ReinforceNullProperties() {
		if ($this->boolDateNull)
			parent::setDate(2000, 1, 1);
		if ($this->boolTimeNull)
	...
	public function IsEqualTo(QDateTime $dttCompare) {
		...
		if ($this->boolDateNull != $dttCompare->boolDateNull)
			return false;

		// If mismatched Time nullstates, then only compare the Date portions
		if ($this->boolTimeNull != $dttCompare->boolTimeNull) {
	...
	public function __set($strName, $mixValue) {
		try {
			switch ($strName) {
				case 'Month':
					if ($this->boolDateNull && (!is_null($mixValue)))
				...
				case 'Hour':
					if ($this->boolTimeNull && (!is_null($mixValue)))
